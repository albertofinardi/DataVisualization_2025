...
class SankeyD3 {
  ...
  // Generate SVG path for a link as a ribbon (filled shape, d3-sankey lib style)
  linkRibbon(d) {
    if (
      !d.sourceNode ||
      !d.targetNode ||
    ...
    if (
      ...
      console.warn("NaN detected in link coordinates:", {
        x0,
        x1,
        y00,
        y01,
  ...
  renderSankey = function (visData, activeDimensions, controllerMethods) {
    ...
    // Remove selected boxes that no longer exist in the new nodes
    ...
    if (removedBoxes.length > 0) {
      console.debug(
        "Removed selected boxes that no longer exist:",
        removedBoxes
      );
    ...
    nodesGroup
      ...
      .join(
        (enter) => {
          ...
          nodeG.style("cursor", "pointer").on("click", (event, d) => {
  ...
  renderDimensionLabels(dimensions) {
    ...
    const labelsGroup = this.matSvg
      .selectAll(".dimension-label-group")
      .data(dimensions, (d, i) => d);

    labelsGroup.join(
      (enter) => {
        const labelG = enter.append("g").attr("class", "dimension-label-group");
  ...
  updateNodeHighlighting() {
    ...
    const selectedIndices = new Set(
      this.currentSelectedItems.map((item) => item.index)
    );
    console.debug(
      "updateNodeHighlighting: selectedIndices size:",
      selectedIndices.size,
      "selectedBoxes size:",
      this.selectedBoxes.size
    );

    this.matSvg.selectAll(".sankey-node").each((d, i, nodes) => {
      const node = d3.select(nodes[i]);

      let matchingCount = 0;
      if (selectedIndices.size > 0) {
        matchingCount = d.houses.filter((house) =>
          selectedIndices.has(house.index)
        ).length;
      }
  ...
  handleSelectionUpdate() {
    ...
    if (this.selectedBoxes.size === 0) {
      ...
      const selectedNodes = this.currentNodes.filter((node) =>
        this.selectedBoxes.has(node.id)
      );
      ...
      console.debug(
        "Selected boxes by dimension:",
        Array.from(nodesByDimension.entries())
          .map(
            ([dim, nodes]) => `${dim}: [${nodes.map((n) => n.name).join(", ")}]`
          )
          .join("; ")
      );
      ...
      if (housesPerDimension.length === 1) {
        ...
        for (let i = 1; i < housesPerDimension.length; i++) {
          ...
          filteredHouses = new Set(
            Array.from(filteredHouses).filter((house) =>
              dimensionIndices.has(house.index)
            )
          );
        }
  ...
  handleNodeClick(nodeData) {
    console.debug("Handling node click:", nodeData.id);
  ...
  highlightSelectedItems(selectedItems) {
    console.debug(
      "Highlighting selected items in Sankey:",
  ...
  highlightFlowsForSelection(selectedItems) {
    ...
    this.matSvg.selectAll(".sankey-link").each(function (linkData) {
      const hasSelectedItems = linkData.houses.some((house) =>
        selectedIndices.has(house.index)
      );
